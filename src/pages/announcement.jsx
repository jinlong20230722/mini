...
export default function Announcement(props) {
  ...
  return <div className="min-h-screen bg-[#F5F7FA]">
      ...
      <div className="px-4 py-6 pb-8">
        {/* 页面说明 - 装饰边框 */}
        <div className="bg-gradient-to-r from-slate-50 to-slate-100 rounded-xl p-4 mb-6 border-l-4 border-slate-700 animate-fade-in-up hover-lift" style={{
        animationDelay: '0.1s'
      }}>
          <div className="flex items-start">
            <div className="bg-gradient-to-br from-slate-700 to-slate-800 rounded-full p-2 mr-3">
              <Megaphone className="w-4 h-4 text-white" />
            </div>
            <div className="flex-1">
              <h4 className="text-sm font-semibold text-slate-800 mb-1 font-['Space_Grotesk']">信息宣发</h4>
              <p className="text-xs text-slate-700 leading-relaxed font-['JetBrains_Mono']">
        ...
        {/* 公告列表 */}
        {announcements.length === 0 ? <div className="bg-white rounded-xl shadow-card p-8 text-center border border-slate-100 animate-fade-in-up" style={{
        animationDelay: '0.2s'
      }}>
            <Megaphone className="w-12 h-12 text-slate-300 mx-auto mb-3" />
            <p className="text-slate-500 text-sm font-['JetBrains_Mono']">暂无公告信息</p>
          </div> : <div className="space-y-4">
            {announcements.map((item, index) => <div key={item._id} className="bg-white rounded-xl shadow-card overflow-hidden border border-slate-100 hover:shadow-card-hover transition-all duration-300 animate-fade-in-up hover-lift" style={{
          animationDelay: `${0.2 + index * 0.1}s`
                ...
                <div className="p-4 cursor-pointer" onClick={() => toggleExpand(item._id)}>
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      ...
                      <h3 className="text-base font-semibold text-slate-800 leading-snug font-['Space_Grotesk']">
                        {item.title}
                      </h3>
                    </div>
                    <div className="ml-2 flex-shrink-0">
                      {expandedItems[item._id] ? <ChevronUp className="w-5 h-5 text-slate-400 transition-transform duration-300" /> : <ChevronDown className="w-5 h-5 text-slate-400 transition-transform duration-300" />}
                    </div>
                  </div>

                  {/* 发布信息 */}
                  <div className="flex items-center text-xs text-slate-500 space-x-4 font-['JetBrains_Mono']">
                    <div className="flex items-center">
                ...
                {expandedItems[item._id] && <div className="px-4 pb-4 pt-0 transition-all duration-300">
                    <div className="border-t border-slate-100 pt-3">
                      <p className="text-sm text-slate-600 leading-relaxed whitespace-pre-wrap font-['JetBrains_Mono']">
                        {item.content}
                      </p>
                    </div>
                  </div>}
